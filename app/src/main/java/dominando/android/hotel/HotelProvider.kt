...
import dominando.android.hotel.repository.sqlite.HotelSqlHelper
...
class HotelProvider : ContentProvider() {
    ...
    override fun getType(uri: Uri): String? {
        return when (sUriMatcher.match(uri)) {
            TYPE_HOTEL_DIR -> "${ContentResolver.CURSOR_DIR_BASE_TYPE}/dominando.android.hotel"
            TYPE_HOTEL_ITEM -> "${ContentResolver.CURSOR_ITEM_BASE_TYPE}/dominando.android.hotel"
            else -> null
        }
    }
    ...
    override fun update(
        ...
        val rowsAffected = when (uriType) {
            TYPE_HOTEL_DIR -> sqlDB.update(TABLE_HOTEL, values, selection, selectionArgs)
            TYPE_HOTEL_ITEM -> {
